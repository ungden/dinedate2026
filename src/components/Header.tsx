...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      ...
      <div className="absolute inset-0 bg-white/80 backdrop-blur-xl border-b border-rose-100/50 supports-[backdrop-filter]:bg-white/60" />
        ...
        <div className="flex items-center justify-between h-[70px]">
          ...
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-2 group">
              <div className="relative flex items-center justify-center">
                 {/* Main Logo Text - Gradient Pink */}
                 <span className="text-2xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-pink-600 group-hover:to-rose-500 transition-all duration-300">DineDate</span>
                 <span className="absolute -top-0.5 -right-2 w-1.5 h-1.5 bg-rose-500 rounded-full animate-pulse"></span>
              </div>
            </Link>

            {/* Desktop Navigation */}
            <nav className="hidden md:flex items-center gap-1 p-1 bg-gray-50/50 rounded-full border border-gray-100">
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn("w-4 h-4", isActive && "stroke-[2.5px] fill-rose-50")} />
                    {link.label}
                    {link.badge ? (
          ...
          <div className="flex items-center gap-3">
            <Link href="/search" className="hidden sm:flex">
                <button className="w-11 h-11 rounded-full bg-white border border-rose-100 hover:border-rose-200 hover:shadow-md flex items-center justify-center text-gray-500 hover:text-rose-500 transition-all">
            ...
            {user ? (
              ...
              <div className="flex items-center gap-3">
                  <Link href="/login" className="hidden sm:block px-4 py-2 text-sm font-bold text-gray-600 hover:text-rose-600 transition-colors">
                    Đăng nhập
                  </Link>
                  <Link href="/register">
